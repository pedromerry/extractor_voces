...
import csv
import ntpath
from repo import audioFeatureExtraction as aF
...
def featureAndTrain(list_of_dirs, mt_win, mt_step, st_win, st_step, 
    ...
    if len(features) == 0:
        print("trainSVM_feature ERROR: No data found in any input folder!writeTr")
        return
    print("features")
    print(features)
...
def featureAndTrainRegression(dir_name, mt_win, mt_step, r, st_step,
    ...
    ARGUMENTS:
        ...
        mt_win, mt_step:        mid-term window length and step
        , st_step:        short-term window and step
...
def fileClassification(inputFile, model_name, model_type):
    ...
    time_result=(numpy.array(range(len(cls))) * mt_step + mt_step / 2.0)
...
def writeTrainDataToARFF(model_name, features, classNames, feature_names):
    ...
    print("enumeration begins")
    print(type(features))
    print(type(features[0]))
    print("enumeration ends")
...
def trainSpeakerModelsScript():
    ...
    dir_name = "pyAudioAnalysis/DIARIZATION_ALL/all"
    ...
    featureAndTrain(list_of_dirs, mt_win, mt_step, st_win, st_step,
                    "knn", "pyAudioAnalysis/data/knnSpeakerAll",
                    compute_beat=False, perTrain=0.50)

    dir_name = "pyAudioAnalysis/DIARIZATION_ALL/female_male"
    list_of_dirs = [os.path.join(dir_name, name)
                    for name in os.listdir(dir_name)
                    if os.path.isdir(os.path.join(dir_name, name))]
    print("list of dirs:")
    print(list_of_dirs)
    featureAndTrain(list_of_dirs, mt_win, mt_step, st_win, st_step,
                    "knn", "pyAudioAnalysis/data/knnSpeakerFemaleMale",
                    compute_beat=False, perTrain=0.50)
